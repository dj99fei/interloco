...
public class DrawerListAdapter extends BaseAdapter<String> {
	private static final int[] ICONS = { R.drawable.ic_drawer_consume,
			R.drawable.ic_drawer_consume };

	private Drawable[] mDrawables = new Drawable[ICONS.length];
	private int mPaddingLeft;
	private int mCompoundDrawablePadding;

	public DrawerListAdapter(Context mContext, List<String> data) {
		super(mContext, data);

		mPaddingLeft = mContext.getResources().getDimensionPixelOffset(
				R.dimen.drawer_item_padding_left);
		mCompoundDrawablePadding = mContext.getResources()
